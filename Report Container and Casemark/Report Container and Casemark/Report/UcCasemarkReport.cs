...
{
    public partial class UcCasemarkReport : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        {
            LblAppName.Text = "Casemark Report";
            InitialConnection();
        ...
        private void GetCasemarkLog()
        {
            try
            {
                ...
                dsActionRequest = dbReport.GetCasemarkReport(Convert.ToDateTime(DteStartDate.EditValue), Convert.ToDateTime(DteEndDate.EditValue));
                ...
                GrdcCasemark.DataSource = dsActionRequest.Tables[0];
                ...
                if (dataColumn.Contains("Seq") && dsActionRequest.Tables[0].Rows.Count > 0)
                {
        ...
        private void ExportExcelFile()
        {
            try
            {
                ...
                if (saveFile.ShowDialog() != DialogResult.Cancel)
                {
                    ...
                    {
                        case ".xls":
                            GrdcCasemark.ExportToXls(exportFilePath);
                            break;
                        case ".xlsx":
                            GrdcCasemark.ExportToXlsx(exportFilePath);
                            break;
                        default:
                            break;
                    }
        ...
        private void ReprintAction()
        {
            try
            {
                ...
                string fileFormat = $@"{pathLabel}\CasemarkLabel.prn";
                ...
                if (!string.IsNullOrEmpty(commandSBPL))
                {
                    ...
                    string printPackingslip = string.Empty;
                    string printCustomername = string.Empty;
                    string printDeliveryplace = string.Empty;
                    string printHub = string.Empty;
                    string printPackagecode = string.Empty;
                    string printWeight = string.Empty;
                    string printVolume = string.Empty;
                    ...
                    printPackingslip = selectedItemDataRow["PrintPackingslip"].ToString();
                    printCustomername = selectedItemDataRow["PrintCustomername"].ToString();
                    printDeliveryplace = selectedItemDataRow["PrintDeliveryplace"].ToString();
                    printHub = selectedItemDataRow["PrintHub"].ToString();
                    printPackagecode = selectedItemDataRow["PrintPackageCode"].ToString();
                    printWeight = selectedItemDataRow["PrintWeight"].ToString();
                    printVolume = selectedItemDataRow["PrintVolume"].ToString();
                    ...
                    if (string.IsNullOrEmpty(printHub))
                    {
                        printHub = " ";
                    }

                    str = commandSBPL;
                    str = str.Replace("@PackingSlip", printPackingslip);
                    str = str.Replace("@Shipment", printShipment);
                    str = str.Replace("@CustomerName", printCustomername);
                    str = str.Replace("@DeliveryPlace", printDeliveryplace);
                    str = str.Replace("@Hub", printHub);
                    str = str.Replace("@PackageCode", printPackagecode);
                    str = str.Replace("@Weight", printWeight);
                    str = str.Replace("@Volume", printVolume);
                    str = str.Replace("@LenQr", printPackingslip.Length.ToString());
        ...
        public UcCasemarkReport()
        {
            InitializeComponent();
            SetDefault();
        }

        private void UcCasemarkReport_Load(object sender, EventArgs e)
        {
            GetCasemarkLog();
        }
